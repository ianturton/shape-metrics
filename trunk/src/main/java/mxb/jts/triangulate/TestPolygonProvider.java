...
public class TestPolygonProvider { 
	...
	static {
		...
		try {
			/*
			 * Triangle (to check that the algorithm doesn't fail when there is
			 * nothing to do
			 */
			poly = (Polygon) reader.read("POLYGON((0 0, 50 100, 100 0, 0 0))");
			 ...
			 */
			poly = (Polygon) reader
					.read("POLYGON( (50 100, 100 200, 200 0, 100 0, 100 100, 50 100) )");

			polygons.put("isoceles", poly);

			/*
			 * Square with extra vertices to test that all vertices get used
			 */
			poly = (Polygon) reader
					.read("POLYGON((0 0, 0 20, 0 40, 0 60, 0 80, 0 100, 100 100, "
			 ...
			 */
			poly = (Polygon) reader
					.read("POLYGON((0 0, 10 0, 10 5, 5 10, 0 5, 0 0), "
							+ "(5 3, 7 5, 5 7, 3 5, 5 3))");
			 ...
			 */
			poly = (Polygon) reader
					.read("POLYGON ((50 440, 50 50, 510 50, 510 440, 280 240, 50 440), "
			 ...
			 */
			poly = (Polygon) reader
					.read("POLYGON ((30 40, 30 180, 210 180, 210 40, 30 40), "
			 ...
			 */
			poly = (Polygon) reader
					.read("POLYGON ((0 0, 0 200, 200 200, 200 0, 0 0), "
			 ...
			 */
			poly = (Polygon) reader
					.read("POLYGON ((0 0, 0 200, 250 200, 250 0, 0 0), "
			...
			try {
				URL url = TestPolygonProvider.class.getResource("resources/badpoly.txt");
				...
				int id=0;
				while(it.hasNext()) {
					poly = (Polygon) reader.read(it.nextLine());
					polygons.put("PA Congressional Dist "+id, poly);
					id++;
				}
