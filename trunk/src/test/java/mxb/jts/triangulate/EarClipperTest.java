...
public class EarClipperTest {
	...
	private void doTest(String polyName) {
		...
		Geometry result = clipper.getResult(false);
		long end = System.currentTimeMillis();
		System.out.println("unimproved Elapsed time "
				+ ((end - start) / 1000.0) + " seconds");
		start = System.currentTimeMillis();
		clipper = new EarClipper(poly);
		result = clipper.getResult(true);
		end = System.currentTimeMillis();
		System.out.println("improved Elapsed time " + ((end - start) / 1000.0)
				+ " seconds");
	...
	/**
	 * Check that none of the polygons in the triangulation result overlap each
	 * other
	 */
	...
	/**
	 * Check that the triangulation result contains all of the polygon vertices
	 * and vice versa
	 */
	...
	/**
	 * Check that the triangulation completely covers the original polygon
	 */
